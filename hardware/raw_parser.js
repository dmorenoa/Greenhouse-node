...
class RawParser extends Transform {
  ...
  if ((parser.pos = data.indexOf(0x23)) !== -1) {
    ...
    parser.payload = data.slice(parser.pos + 4,  (parser.pos + 5 + parser.len));
    parser.checksum = data[parser.pos + 6 + parser.len];

    if(parser.payload.length == parser.len + 4){
      var buff = new Buffer(parser.len);
      parser.payload.slice(2, parser.payload.length-1).copy(buff);
      var sum = 0;
